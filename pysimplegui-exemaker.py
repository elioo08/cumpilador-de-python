...
def main():
    ...
    layout = [[sg.Text('Cumpilador de python     ', font='Any 15')],
              [sg.Text('Local do arquivo python: '), sg.Input(key='-sourcefile-', size=(45, 1)),
               sg.FileBrowse(file_types=(("Python Files", "*.py"),))],
              [sg.Text('Icone (Opcional)'), sg.Input(key='-iconfile-', size=(45, 1)),
               sg.FileBrowse(file_types=(("Icon Files", "*.ico"),))],
              [sg.Frame('Saida', font='Any 15', layout=[
                        [sg.Output(size=(65, 15), font='Courier 10')]])],
              [sg.Button('Criar .EXE', bind_return_key=True),
               sg.Button('Cancelar', button_color=('white', 'firebrick3')) ],
              [sg.Text('Criado com PySimpleGUI (www.PySimpleGUI.org). Traduzido por Elio Benedicto', auto_size_text=True, font='Courier 8')]]

    window = sg.Window('Cumpilador de Python', layout, auto_size_text=False, auto_size_buttons=False, default_element_size=(20,1), text_justification='right')
    # ---===--- Loop taking in user input --- #
    while True:

        event, values = window.read()
        if event in ('Exit', 'Cancelar', None):
            break
        ...
        if event == 'Criar .EXE':
            try:
                ...
                sg.PopupError('Algo deu errado', 'close this window and copy command line from text printed out in main window','Here is the output from the run', out)
